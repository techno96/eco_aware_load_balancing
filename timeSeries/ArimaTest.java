...
public class ArimaTest {
    ...
    public double[] commonTestSimpleForecast(final String name, final double[] trainingData,
        ...
        try
        {
        fos=new FileOutputStream("G:\\Final Year Proj\\Dataset\\Prediction.txt");
        }
        catch(Exception e)
        {
            Log.printLine(e);
        ...
        for (int i = 0; i < forecast.length; ++i) {
            sb.append(dbl2str(trueForecastData[i])).append("    | ")
              .append(dbl2str(lower[i])).append("   ").append(dbl2str(Math.floor(forecast[i])))
              .append("   ").append(dbl2str(upper[i]))
              .append("\n");
            try
            {
            String vv=df.format(trueForecastData[i]);
            fos.write(vv.getBytes());
            vv="\t";
            fos.write(vv.getBytes());
            vv=df.format(Math.floor(forecast[i]));
            fos.write(vv.getBytes());
            vv="\r\n";
            fos.write(vv.getBytes());
            }
        ...
        double temp = 0.0;
        for (int i = 0; i < forecast.length; ++i) {
            temp += Math.pow(Math.floor(forecast[i]) - trueForecastData[i], 2);
        }
