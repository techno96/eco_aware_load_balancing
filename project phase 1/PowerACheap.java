...
public class PowerACheap {
	...
	private static List<Vm> vmlist;
        ...
        {      
	            	...
	            	boolean trace_flag = true;  // mean trace events
                        ...
                        for(int i=0;i<m;i++)
                        {
                            String g="F:\\Final Year Proj\\Dataset\\";
                        ...
                        Scanner sc4=new Scanner(new FileInputStream(b));
                            ...
                            a=sc4.next();
                            solar[i]=Double.parseDouble(sc4.next())*0.67*200*0.40*100/(1000*3600);
                            Log.printLine(solar[i]+"\t"+wind[i]+"\t"+grid[i]);
                        }
	            	...
	            	vm[i]= new Vm(i, brokerId, mips, pesNumber, ram, bw, size, vmm, new CloudletSchedulerTimeShared());
                        ...
                        int index=0;double maxval=9999999;
                        
                        for(int j=0;j<m;j++)
                        {
                            if(datacenter_power_cost[j]<maxval)
                            {
                                index=j;
                                maxval=datacenter_power_cost[j];
                            }
                        }
	            	...
	            	UtilizationModel utilizationModel = new UtilizationModelFull();
                        ...
                        int cloudlet_datacenter[]=new int[500];
	            	...
	            	cloudlet[i].setUserId(brokerId);
                        ...
                        cloudlet[i].setVmId(index);
                        
                        cloudlet_datacenter[i]=index;
	            	...
	            	printCloudletList(newList,datacenter_power_cost,solar,wind,grid,cloudlet_datacenter,m);
	...
	private static DatacenterBroker createBroker(int id){
        ...
        private static void printCloudletList(List<Cloudlet> list,double cost[],double solar[],double wind[],double grid[],int cloudlet_datacenter[],int m) {
	        int size = list.size();
	        Cloudlet cloudlet;
	        Log.printLine();
...
try
{
                FileOutputStream fs=new FileOutputStream(new File("F:\\Final Year Proj\\Dataset\\PowerACheap.txt"));
	        ...
	        for (int i = 0; i < size; i++) {
	            ...
	            Log.print(cloudlet.getCloudletId() + "\t\t");
                    ...
                    if(renewable[cloudlet_datacenter[cloudlet.getCloudletId()]]>power_consumed)
                    {
                        cloudlet_cost=0.0;
                        renewable[cloudlet_datacenter[cloudlet.getCloudletId()]]-=power_consumed;
                    } 
                    else
                    {
                        cloudlet_cost=power_consumed*cost[cloudlet_datacenter[cloudlet.getCloudletId()]];
                        renewable[cloudlet_datacenter[cloudlet.getCloudletId()]]-=power_consumed;
                    }

	            

	            	Log.print( "SUCCESS\t\t\t" + /*cloudlet_datacenter[cloudlet.getCloudletId()]*/cloudlet.getResourceId() + "\t\t"+ dft.format(cloudlet.getSubmissionTime())+"\t\t"+dft.format(cloudlet.getActualCPUTime()) + "\t\t"+ dft.format(cloudlet.getExecStartTime())+
                        ...
                        Log.print(dft2.format(power_consumed)+"\t\t"+dft2.format(cloudlet_cost));
                        Log.print("\n");
