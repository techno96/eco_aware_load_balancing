...
class DatacenterDistance
{
    int distance;
    int queue_length;
}

public class CloudCheapestFirstScheduling {
	...
	public static void main(String[] args) {
        ...
        List<Cloudlet> cList10=new ArrayList<Cloudlet>();
                        ...
                        int dtdis[]=new int[m];
                        final float dtcost[]=new float[m];
                        final BufferedReader br=new BufferedReader(new InputStreamReader(new FileInputStream("F:\\Final Year Proj\\Dataset\\file1.txt")));
                        Random r=new Random();
                        for(int i=0;i<m;i++)
                        {
                            ...
                            dtcost[i]=Float.parseFloat(br.readLine());
			...
			UtilizationModel utilizationModel = new UtilizationModelFull();
                        ...
                        final int broker[]=new int[m];
                        final DatacenterBroker dcb[]=new DatacenterBroker[m];
                        
                        for(int i=0;i<m;i++)
                        {
                            dcb[i]=createBroker(i+1);
                            broker[i]=dcb[i].getId();
                            vmList =createVM(broker[i],1,0);
                        ...
                        int index=0;float maxval=9999999;
                        for(int j=0;j<m;j++)
                        {
                            if(dtcost[j]<maxval)
                            {
		...
		Runnable monitor = new Runnable() {
				...
				public void run(){
					...
					CloudletList = createCloudlet(broker[index], 10,20); // creating 10 cloudlets

				
				       dcb[index].submitCloudletList(CloudletList);
	...
	private static void printCloudletList(List<Cloudlet> list) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS){
				...
				Log.printLine( indent + indent + (cloudlet.getResourceId()-1) + indent + indent + indent + cloudlet.getVmId() +
